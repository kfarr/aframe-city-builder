...
AFRAME.registerComponent('builder-controls', {
  ...
  addEventListeners: function () {
    ...
    this.el.addEventListener('axismove', function (evt) {
      ...
      // var size = (evt.detail.axis[1] + 1) / 2 * self.schema.size.max;  // example from a-painter
      ...
      if (evt.detail.axis[0] > 0) {
        // right arrow green hover
        if (new THREE.Color(arrowRight.getAttribute("material").color).r === 0) { // if not already yellow (which indicates button press) then indicate green hover
          arrowRight.removeAttribute('animation__color');
          arrowRight.removeAttribute('animation__opacity');
          arrowRight.setAttribute('animation__color', { property: 'material.color', dur: 500, from: "#00FF00", to: "#000000" });
          arrowRight.setAttribute('animation__opacity', { property: 'material.opacity', dur: 500, from: "1", to: "0.5" });
        }
      } else {
        // left arrow green hover
        if (new THREE.Color(arrowLeft.getAttribute("material").color).r === 0) { // if not already yellow (which indicates button press) then indicate green hover
          arrowLeft.removeAttribute('animation__color');
          arrowLeft.removeAttribute('animation__opacity');
          arrowLeft.setAttribute('animation__color', { property: 'material.color', dur: 500, from: "#00FF00", to: "#000000" });
  ...
  onPlaceObject: function () {
    ...
    if (this.el.id === 'rightController') {
      ...
      if (leftButton) {
        ...
        var arrowRight = document.getElementById("arrowLeft");
        arrowLeft.removeAttribute('animation__color');
        arrowLeft.removeAttribute('animation__opacity');
