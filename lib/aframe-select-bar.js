...
AFRAME.registerComponent('select-bar', {
  ...
  init: function () {
    ...
    function makeSelectOptionsRow(selectedOptgroupEl, parentEl, index, offsetY = 0) {
    ...
    makeSelectOptionsRow(selectedOptgroupEl, selectRenderEl, this.data.selectedOptgroupIndex);

//    var nextSelectedOptgroupEl = optgroups[this.data.selectedOptgroupIndex + 1];  // fetch the first optgroup
//    makeSelectOptionsRow(nextSelectedOptgroupEl, selectRenderEl, this.data.selectedOptgroupIndex + 1, -0.15);
...
//    el.removeEventListener('onOptgroupPrevious', this.onOptgroupPrevious);
  ...
  onTrackpadDown: function (evt) {
    ...
    var direction = leftButton ? "previous" : "next";
    // console.log("leftButton? " + leftButton);
    // console.log("direction? " + direction);

    // TODO: this should reflect the component parameter passed - see this.data.controller (sp?)
    //menu: only deal with trackpad events from right controller
    if (evt.target.id === 'leftController') {
      return;
    }

    this.onOptionSwitch(direction);
