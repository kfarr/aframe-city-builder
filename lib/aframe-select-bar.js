...
AFRAME.registerComponent('select-bar', {
  ...
  makeSelectOptionsRow: function(selectedOptgroupEl, parentEl, index, offsetY = 0) {
...
//    el.removeEventListener('onOptgroupPrevious', this.onOptgroupPrevious);
  ...
  onAxisMove: function (evt) {       // menu: used for determining current axis of trackpad hover position
    if (evt.target.id != this.data.controllerID) {   //menu: only deal with trackpad events from right controller
      return;
    }
...
//    console.log(evt.detail.axis[0]);
    if (Math.abs(evt.detail.axis[0]) > Math.abs(evt.detail.axis[1])) { // if x axis absolute value (left/right) is greater than y axis (down/up)
      ...
      if (evt.detail.axis[1] > 0) { // if the up axis is greater than 0 (midpoint)
        this.onHoverUp();
        console.log("hoverUp");
      } else {
        this.onHoverDown();
        console.log("hoverDown");
      }
    }
  ...
  onOptgroupNext: function(evt) {
    ...
    if (this.data.selectedOptgroupIndex + 2 > optgroups.length) {
      ...
      arrowDown.removeAttribute('animation__color');
      arrowDown.removeAttribute('animation__opacity');
      arrowDown.removeAttribute('animation__scale');
      arrowDown.setAttribute('animation__color', { property: 'material.color', dur: 500, from: "#FF0000", to: "#000000" });
      arrowDown.setAttribute('animation__opacity', { property: 'material.opacity', dur: 500, from: "1", to: "0.5" });
      arrowDown.setAttribute('animation__scale', { property: 'scale', dur: 500, from: "-0.006 0.003 0.006", to: "-0.004 0.002 0.004" });
      ...
      var nextSelectedOptgroupEl = optgroups[this.data.selectedOptgroupIndex];  // fetch the first optgroup
      ...
      arrowDown.removeAttribute('animation__opacity');
      arrowDown.removeAttribute('animation__scale');
      arrowDown.setAttribute('animation__color', { property: 'material.color', dur: 500, from: "#FFFF00", to: "#000000" });
      arrowDown.setAttribute('animation__opacity', { property: 'material.opacity', dur: 500, from: "1", to: "0.5" });
      arrowDown.setAttribute('animation__scale', { property: 'scale', dur: 500, from: "-0.006 0.003 0.006", to: "-0.004 0.002 0.004" });
    }
  ...
  onTrackpadDown: function (evt) {
    ...
    var arrowColorArray = [arrowUpColor.g, arrowRightColor.g, arrowDownColor.g, arrowLeftColor.g];

    console.log("trackpad down");
    console.log(arrowColorArray);
    // Determine which value in the array is the largest
    switch (indexOfMax(arrowColorArray)) {
      case 0:        // up
        console.log("PRESSup");
        return;
      case 1:        // right
  ...
  onOptionSwitch: function (direction) {
    ...
    // console.log(direction);
    selectOptionsRowEl = document.getElementById('selectOptionsRow' + this.data.selectedOptgroupIndex)
