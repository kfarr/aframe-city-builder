...
//      console.log(element.getAttribute("value"));
      ...
      <a-entity id="menu${element.getAttribute("index")}" visible="${visible}" class="preview${ (selected) ? " selected" : ""}" optionid="${element.getAttribute("index")}" value="${element.getAttribute("value")}" optgroup="${selectedOptgroup.getAttribute("value")}" position="${startPositionX} 0 0">
        ...
        <a-entity class="objectName" position="-0.025 -0.04 -0.003" scale="0.05 0.05 0.05" bmfont-text="text: ${element.text}; color: #747474"></a-entity>
      </a-entity>`
  ...
  removeEventListeners: function () {
    var el = this.el;
    el.removeEventListener('trackpaddown', this.onOptionSwitch);
  },
  ...
  onOptionSwitch: function (evt) {
    ...
    if (leftButton) {
      ...
      if (selectedOptionIndex == -1) {selectedOptionIndex = selectedOptgroupEl.childElementCount - 1}
      ...
      if (newlyVisibleOptionIndex < 0) {newlyVisibleOptionIndex = selectedOptgroupEl.childElementCount - 3 + oldSelectedOptionIndex};
      ...
      if (newlyRemovedOptionIndex > selectedOptgroupEl.childElementCount) {newlyRemovedOptionIndex = oldSelectedOptionIndex + 3 - selectedOptgroupEl.childElementCount};
      ...
      if (newlyInvisibleOptionIndex > selectedOptgroupEl.childElementCount) {newlyInvisibleOptionIndex = oldSelectedOptionIndex + 2 - selectedOptgroupEl.childElementCount};
...
//      var negFourthMenuObjectID = (objectId - 3 > -1) ? (objectId - 3) : (objectId - 3 + objectArray.length);
      ...
      if (newlyCreatedOptionIndex < 0) {newlyCreatedOptionIndex = selectedOptgroupEl.childElementCount - 4 + oldSelectedOptionIndex};
...
//      selectOptionsRowEl.appendChild(negFourthMenuEl);
//      selectOptionsRowEl.insertBefore( negFourthMenuEl, selectOptionsRowEl.firstChild );
...
//      var appendedNewlyCreatedOptionEl = $('#menu' + (newlyCreatedOptionIndex));
      ...
      // appendedNewlyCreatedOptionEl.children(".previewImage")[0].setAttribute('src', 'assets/preview/' + objectArray[negFourthMenuObjectID].file + '.jpg')
      // appendedNewlyCreatedOptionEl.children(".objectName")[0].setAttribute('bmfont-text', 'text', humanize(objectArray[negFourthMenuObjectID].file));
      // appendedNewlyCreatedOptionEl.children(".objectName")[0].setAttribute('bmfont-text', 'color', '#747474');
...
// 12:45 20mL
    ...
    // LEFT MENU END ===============================
      ...
      const selectOptionsRowEl = document.querySelector("#menuObjectList");
      // use the desiredPosition attribute (if exists) instead of object3D position as animation may not be done yet
      if (selectOptionsRowEl.hasAttribute("desiredPosition")) {
        var oldPosition = selectOptionsRowEl.getAttribute("desiredPosition");
        var newX = parseFloat(oldPosition.split(" ")[0]) - 0.075;
      ...
      selectOptionsRowEl.appendChild(fourthMenuEl);
