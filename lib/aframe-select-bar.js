...
AFRAME.registerComponent('select-bar', {
  schema: {
    ...
    selectedOptgroupValue: {type: 'string'},
    selectedOptionValue: {type: 'string'},
    selectedOptionIndex: {type: 'string'}
  },
...
//    console.log(optGroups);
    selectedOptgroup = optGroups['mmmm_alien']; //TODO - temporary, this should be browseable
    this.data.selectedOptgroupValue = 'mmmm_alien'; //TODO - temporary, this should be browseable
    ...
    menuArray.forEach(function (element, menuArrayIndex) {
      ...
      selectOptionsHTML += `
      <a-entity id="menu${originalOptionsArrayIndex}" visible="${visible}" class="preview${ (selected) ? " selected" : ""}" optionid="${originalOptionsArrayIndex}" value="${element.getAttribute("value")}" optgroup="${selectedOptgroup.getAttribute("value")}" position="${startPositionX} 0 0">
