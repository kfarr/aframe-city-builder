...
/***/ function(module, exports, __webpack_require__) {
	...
	var itemCount = 0; // scene starts with 0 items
	var bases = [
	...
	var objects = [
		{ file: 'veh_ambulance', scale: '0.005 0.005 0.005' }, //0
		{ file: 'obj_treekf', scale: '0.01 0.01 0.01' } //1
	];
	...
	AFRAME.registerComponent('demo-controls', {
	  ...
	  onTriggerDown: function () {
	    ...
	    var controllerWorldPosition = this.el.object3D.getWorldPosition();
			...
			var roundedPositionString = roundedControllerPositionX + ' ' + roundedControllerPositionY + ' ' + roundedControllerPositionZ;
			...
			if(thisItemEl.attributes.objectType.value !== 'bases'){ return; };

			// experimental - check to see if there is already an object here with this value
			// var existingItem = document.querySelectorAll('[position="' + roundedPositionString + '"]');
			// console.log(existingItem);

	    sceneEl.append('a-obj-model')
	           .attr('id', 'item' + itemCount)
						 .attr('class', 'item base children')
	           .attr('scale', bases[objectId].scale)
						 .attr('position', roundedPositionString)
						 .attr('rotation', roundedEulerRotationString)
						.attr('src', '#' + bases[objectId].file + '-obj')
						.attr('mtl', '#' + bases[objectId].file + '-mtl');
						 	          //  .attr('class', 'throwable')
						 						//  .attr('dynamic-body', '') // make it physic-like

			var querything = "#item" + itemCount;
			console.log(querything);

			var childBase = d3.select(querything);

			console.log(childBase);

			if(typeof childBase !== "undefined"){
				childBase.transition()
		         .duration(500)
	    ...
	    var controllerID = controllerEl.attr('id');
			...
			var objectId = parseInt(thisItemEl.attributes.objectId.value);
			objectId += 1;
			if (objectId == bases.length) {objectId = 0}
			// console.log(bases);
			// console.log(objectId);

			// Set this plate to be whatever is currently the "preview" item
			thisItemEl.setAttribute('obj-model', 'obj', '#' + bases[objectId].file + '-obj');
			thisItemEl.setAttribute('obj-model', 'mtl', '#' + bases[objectId].file + '-mtl');
