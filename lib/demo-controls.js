...
/***/ function(module, exports, __webpack_require__) {
	...
	// commented out the grid snapping indicator as it caused performance problems
	...
	//
	// 			// var intersectedEl = d3.select(this.el);
	 ...
	 */
	AFRAME.registerComponent('demo-controls', {
	  schema: {},
	  ...
	  onTriggerDown: function () {
	    ...
	    var controllerWorldPosition = this.el.object3D.getWorldPosition();
			...
			var roundedControllerPositionX = Math.round(thisItemWorldPosition.x * 2) / 2; //round to nearest 0.5 for ghetto "snapping"
			var roundedControllerPositionY = Math.round(thisItemWorldPosition.y * 2) / 2; //round to nearest 0.5 for ghetto "snapping"
			var roundedControllerPositionZ = Math.round(thisItemWorldPosition.z * 2) / 2; //round to nearest 0.5 for ghetto "snapping"

			var baseId = parseInt(thisItemEl.attributes.baseid.value);

			// console.log(thisItemWorldRotation);

	    sceneEl.append('a-obj-model')
	           .attr('id', 'base-children')
	           .attr('scale', '0.01 0.01 0.01')
						 .attr('position', roundedControllerPositionX + ' ' + roundedControllerPositionY + ' ' + roundedControllerPositionZ)
						 .attr('rotation', function() {
						   ...
						   var thisItemWorldRotationZ = thisItemWorldRotation._z / (Math.PI / 180);
							 var thisItemWorldRotationX = 0; // ignore roll
							 var thisItemWorldRotationZ = 0; // ignore pitch
							 var roundedThisItemWorldRotationY = Math.round(thisItemWorldRotationY / 90) * 90;
							 var eulerRotationString = thisItemWorldRotationX + ' ' + roundedThisItemWorldRotationY + ' ' + thisItemWorldRotationZ;
							//  console.log(eulerRotationString);
						   return eulerRotationString;
						 })
						.attr('src', '#' + bases[baseId].file + '-obj')
						.attr('mtl', '#' + bases[baseId].file + '-mtl');
						 	          //  .attr('class', 'throwable')
						 						//  .attr('dynamic-body', '') // make it physic-like
	  },

		onTrackpadDown: function () {
			// switch between the available bases
	    ...
	    var controllerID = controllerEl.attr('id');
			...
			var baseId = parseInt(thisItemEl.attributes.baseid.value);
			baseId += 1;
			if (baseId == bases.length) {baseId = 0}
			// console.log(bases);
			// console.log(baseId);

			// Set this plate to be whatever is currently the "preview" item
			thisItemEl.setAttribute('obj-model', 'obj', '#' + bases[baseId].file + '-obj');
			thisItemEl.setAttribute('obj-model', 'mtl', '#' + bases[baseId].file + '-mtl');
			thisItemEl.setAttribute('baseid', baseId);
			// base-id="0"

		},

	  /**
	   * Cleans the table by rotating it 90 degrees.
	   */
	  onMenuDown: function () {
	    var table = d3.select('#tableTop');

	    table.transition()
	         .duration(2000)
	         .attrTween('rotation', function() {
	           return d3.interpolate('0 0 0', '0 0 90');
	         })
	         .transition()
	           .duration(2000)
	           .attrTween('rotation', function() {
	             return d3.interpolate('0 0 90', '0 0 0');
	           });
	  },

	  /**
	   * Summons a stick (for reach)
	   *
	   */
	  onGripDown: function () {
	    var controllerEl = d3.select(this.el);
